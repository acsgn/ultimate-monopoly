		...
		// ref: cards "Changing Lanes", the alteration with 'below'
		 if (name.equalsIgnoreCase("Changing Lanes - Below")) {
			...
			if(name.equals("Buyer's Market")) {
				...
				// TODO send player a message to choose any of the
				//		squares that fit the criteria
				//Move to any Unowned Outer Track property 
				//Buy it from the Bank for 1/2 Price.
			...
			if(name.equals("Foreclosed Property Sale!")) {
				// TODO Send a message to choose another player, get their
				//		property.
				//Foreclose on any opponent's mortgaged property. 
				//Pay the mortgage value to the bank to claim the property.
			}
			...
			if(name.equals("See you in Court")) {
				// TODO Send a message to choose another player, have the 
				//		other player pay 250$ to the current player.
				//Sue any player for unfair business practices. 
				//Take $250 from any player of your choice.
			}
			...
			// ref: card "Advance to the Nearest Railroad"
			// !!! Name of this particular card has been shortened in the code.
			else if (name.equalsIgnoreCase("Nearest Railroad")) {
				
				if (player.getCurrentTrack().equals(TrackType.INNER_TRACK)) {
					
					// There is only one chance square in the inner track, so we
					// can immediately proceed to the Railroad at inner.21 without
					// checking anything else.
					player.goTo(TrackType.INNER_TRACK, 21);;
					
				} else if (player.getCurrentTrack().equals(TrackType.MIDDLE_TRACK)) {
					
					if (player.getPosition() == 7) {
						player.goTo(TrackType.MIDDLE_TRACK, 5);
					} else if (player.getPosition() == 22) {
						player.goTo(TrackType.MIDDLE_TRACK, 25);
					} else if (player.getPosition() == 36) {
						player.goTo(TrackType.MIDDLE_TRACK, 35);
					}
					
				} else if (player.getCurrentTrack().equals(TrackType.OUTER_TRACK)) {
					
					if (player.getPosition() == 10) {
						player.goTo(TrackType.OUTER_TRACK, 7);
					} else if (player.getPosition() == 21) {
						player.goTo(TrackType.MIDDLE_TRACK, 15);
					} else if (player.getPosition() == 30) {
						player.goTo(TrackType.OUTER_TRACK, 35);
					} else if (player.getPosition() == 54) {
						player.goTo(TrackType.MIDDLE_TRACK, 35);
					}
					
				}
				
				// re-paying the rent since the card requires double the payment
				if (player.getLocation().isOwned()) { player.payRent(player.getLocation()); }
